...
t = 0:1e-5:2/f; % Vetor de tempo (1 período)

%% 6. Correntes desequilibradas (Eq. 2)
ias = 0.905*sqrt(2) * cos(w*t);
ibs = 0.707*sqrt(2) * cos(w*t - deg2rad(120));
ics = - (ias + ibs); % Corrente da fase c (estrela sem neutro)
...
title('Correntes de Fase no Estator (Desequilibradas)');

subplot(2,1,2);
plot(t, ialpha, 'k', t, ibeta, 'm'); grid on;
xlabel('Tempo (s)'); ylabel('Corrente (A)');
legend('i_{\alpha s}(t)', 'i_{\beta s}(t)', 'Location', 'best');
title('Correntes no Referencial \alpha\beta (Desequilibradas)');

% Trajetória no plano αβ (Figura 3)
figure(2);
plot(ialpha, ibeta); grid on;
xlabel('i_{\alpha s} (A)'); ylabel('i_{\beta s} (A)');
title('Trajetória no Plano \alpha\beta (Desequilibradas)');

% Cálculo das FMMs (Figura 4)
...
title('Forças Magnetomotrizes no Estator (Desequilibradas, t=0)');

% 6. Animação da FMM girante ao longo do tempo (versão simplificada)
figure(4);

for k = 1:20:length(t) % Passo de 50 para animação mais rápida
